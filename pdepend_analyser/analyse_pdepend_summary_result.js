					...
					if (temp_method == "object") {
						...
						if (temp_method_len != "undefined") {
							my_class['method'].forEach(function(my_method) {
								print(my_class['@name'] + "," + my_method['@name'] + "," + my_method['@ccn'] + "," + my_method['@ccn2']);
							})
						}
						// if the 'method' is k-v object, no need 'forEach' function, could get the value directly
						else {
							print(my_class['@name'] + "," + my_class['method']['@name'] + "," + my_class['method']['@ccn'] + "," + my_class['method']['@ccn2']);
						}
					}
				})
		}
			...
			if (temp_method == "object") {
				...
				if (temp_method_len != "undefined") {
					my_package['class']['method'].forEach(function(my_method) {
						print(my_package['class']['@name'] + "," + my_method['@name'] + "," + my_method['@ccn'] + "," + my_method['@ccn2']);
					})
				}
				// if the 'method' is k-v object, no need 'forEach' function, could get the value directly
				else {
					print(my_package['class']['@name'] + "," + my_package['class']['method']['@name'] + "," + my_package['class']['method']['@ccn'] + "," + my_package['class']['method']['@ccn2']);
				}
			}
